...
| name             | text                                                   | Auto-generated                    | ✖        | A unique name for the vectorizer. If not provided, it's auto-generated based on the destination type:<br>- For table destination: `[target_schema]_[target_table]`</br><br>- For column destination: `[source_schema]_[source_table]_[embedding_column]`</br><br>Must follow snake_case pattern `^[a-z][a-z_0-9]*$` </br>                           |
| destination      | [Destination configuration](#destination-configuration)| `ai.destination_table()`          | ✖        | Configure how the embeddings will be stored. Two options available: <ul><li>`ai.destination_table()` (default): Creates a separate table to store embeddings</li><li> `ai.destination_column()`: Adds an embedding column directly to the source table</li></ul>                             |
